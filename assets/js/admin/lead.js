...
async function loadLeads() {
  ...
  try {
      ...
      // Wait for all promises to resolve and generate the table rows
      const leadRows = await Promise.all(leads.map(async (lead) => {
        // Fetch the createdBy user's name
        ...
        return `
          <tr>
              ...
              <td>${lead.assignedToSummary ? lead.assignedToSummary.name : 'N/A'}</td>
              <td>${createdByName}</td> <!-- Added Created By column -->
              <td>
                  ${lead.action !== 'ASSIGNED' 
                   ...
                   <button onclick='showUpdateLeadModal(${JSON.stringify(lead)})'>Update</button>
              </td>
          </tr>
        `;
      }));

      // Now insert the lead rows into the table
...
async function showAssignmentModal(leadId) {
  try {
    ...
    const response = await fetch("http://178.16.137.180:8080/api/users");
...
// Function to assign the lead to a selected user
async function assignLead(leadId, userId, userName) {
  try {
    const response = await fetch(`http://178.16.137.180:8080/api/leads/${leadId}/assign/${userId}`, {
      method: "PUT"
    });
    ...
    if (!response.ok) {
      ...
      // Optional: if backend returns a specific message for invalid status, handle it here
    ...
    customAlert(`Lead #${leadId} successfully assigned to ${userName} and send notification`);
    await loadLeads();
  } catch (error) {
...
// Function to unassign a lead from an employee
async function unassignLead(leadId) {
  try {
      const response = await fetch(`http://178.16.137.180:8080/api/leads/${leadId}/unassign`, {
          method: "PUT"
      });

      if (!response.ok) {
          const errorText = await response.text(); // Consistent with backend string message
          throw new Error(errorText || "Unassignment failed");
      }

      customAlert("Lead has been successfully unassigned and send Notification");
      await loadLeads();
  } catch (error) {
...
async function deleteLead(id) {
  ...
  try {
      ...
      const response = await fetch(`http://178.16.137.180:8080/api/leads/${id}`, {
          method: "DELETE"
      });
    ...
    <div class="modal" role="dialog" aria-modal="true">
      <div class="modal-content">
        ...
        <form id="updateLeadForm">
          ...
          <input  id="updateBudget" value="${lead.budget}" placeholder="Budget" step="0.01" />
...
// async function loadLeads(userId = null) {
//   const tbody = document.querySelector("#leadTable tbody");
//   const loading = document.getElementById("tableLoading");
...
//       const assignedResponse = await fetch(`http://178.16.137.180:8080/api/leads/assigned-to/${userId}`);
//       const assignedLeads = await assignedResponse.json();

//       // Fetch leads created by user
//       const createdResponse = await fetch(`http://178.16.137.180:8080/api/leads/created-by/${userId}`);
//       const createdLeads = await createdResponse.json();

//       // Merge both (remove duplicates if needed)
//       const combinedLeads = [...assignedLeads, ...createdLeads];

//       // Remove duplicate leads (optional but recommended)
//       const uniqueLeadsMap = new Map();
//       combinedLeads.forEach(lead => {
//         uniqueLeadsMap.set(lead.id, lead); // Assuming lead.id is unique
//       });

//       leads = Array.from(uniqueLeadsMap.values());
//     } else {
//       // If no userId provided, fetch all leads
//       const allResponse = await fetch("http://178.16.137.180:8080/api/leads");
//       leads = await allResponse.json();
//     }
...
//     } else {
//       const row = document.createElement("tr");
//       row.innerHTML = `<td colspan="10" style="text-align:center;">No leads found</td>`;
//       tbody.appendChild(row);
//     }
