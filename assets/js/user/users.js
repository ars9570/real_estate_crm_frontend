...
async function getDashboard() {
  ...
  try {
    ...
    // Fetch leads assigned to and created by the user
    ...
    if (!assignedResponse.ok) throw new Error('Failed to fetch assigned leads');
    if (!createdResponse.ok) throw new Error('Failed to fetch created leads');

    const assignedLeads = await assignedResponse.json();
    const createdLeads = await createdResponse.json();

    // Combine both and remove duplicates by leadId
    const allLeadsMap = new Map();
    [...assignedLeads, ...createdLeads].forEach(lead => {
      allLeadsMap.set(lead.leadId, lead);
    });
    const uniqueLeads = Array.from(allLeadsMap.values());

    const totalLeads = uniqueLeads.length;
    const dealsClosed = uniqueLeads.filter(lead => lead.status === 'CLOSED').length;

    // Fetch all properties
    const propertiesResponse = await fetch(`http://178.16.137.180:8080/api/properties/created-by/${userId}`);
